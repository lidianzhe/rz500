...
Q_DECLARE_METATYPE(Neurotec::Plugins::NPlugin*)
...
private:
	...
	QString _param;
        ...
        AsyncRun(void (Neurotec::Plugins::NPluginManager::*ptr)(), Neurotec::Plugins::NPluginManager * instance, QObject * parent = NULL ) :
		QThread(parent)
	{
        ...
        AsyncRun(void (Neurotec::Plugins::NPlugin::*ptr)(), Neurotec::Plugins::NPlugin * plugin, QObject * parent = NULL) : QThread(parent)
	{
		_pluginFunction = ptr;
		_plugin = plugin;
		_target = PluginFunc;
	}
        AsyncRun(Neurotec::Plugins::NPluginManager* manager, QString param, QObject * parent = NULL) :
		QThread(parent)
	{
...
//		}
        ...
        static void AddPluginAsync(Neurotec::Plugins::NPluginManager * pManager, QString plugin, QObject * parent)
	{
		AsyncRun * target = new AsyncRun(pManager, plugin, parent);
		target->start();
	}
        static void PlugAllAsync(Neurotec::Plugins::NPluginManager * pManager, QObject * parent)
	{
                AsyncRun * target = new AsyncRun(&Neurotec::Plugins::NPluginManager::PlugAll, pManager, parent);
		target->run();
	}
        static void UnplugAllAsync(Neurotec::Plugins::NPluginManager * pManager, QObject * parent)
	{
                AsyncRun * target = new AsyncRun(&Neurotec::Plugins::NPluginManager::UnplugAll, pManager, parent);
		target->run();
	}
        static void RefreshAsync(Neurotec::Plugins::NPluginManager * pManager, QObject * parent)
	{
                AsyncRun * target = new AsyncRun(&Neurotec::Plugins::NPluginManager::Refresh, pManager, parent);
		target->run();
	}
        static void PlugAsync(Neurotec::Plugins::NPlugin * plugin, QObject * parent)
	{
                AsyncRun * target = new AsyncRun(&Neurotec::Plugins::NPlugin::Plug, plugin, parent);
		target->start();
	}
        static void UnplugAsync(Neurotec::Plugins::NPlugin * plugin, QObject * parent)
	{
                AsyncRun * target = new AsyncRun(&Neurotec::Plugins::NPlugin::Unplug, plugin, parent);
		target->start();
	}
        static void EbableAsync(Neurotec::Plugins::NPlugin * plugin, QObject * parent)
	{
                AsyncRun * target = new AsyncRun(&Neurotec::Plugins::NPlugin::Enable, plugin, parent);
		target->start();
	}
        static void DisableAsync(Neurotec::Plugins::NPlugin * plugin, QObject * parent)
	{
                AsyncRun * target = new AsyncRun(&Neurotec::Plugins::NPlugin::Disable, plugin, parent);
		target->start();
	}
...
public:
        explicit QPluginManagerDialog(Neurotec::Plugins::NPluginManager * pManager, Neurotec::Plugins::NPlugin * selectedPlugin = NULL, QString title = QString(), QWidget *parent = 0)
		: QDialog(parent)
	{
		setupUi();
		...
		OnSetSelectedPlugin(selectedPlugin);
                qRegisterMetaType<Neurotec::Plugins::NPlugin*>();
	}
...
protected:
	void closeEvent(QCloseEvent *)
	{
		if(pluginManager)
		{
			...
			{
                                Neurotec::Plugins::NPlugin * plugin = (Neurotec::Plugins::NPlugin*)model->item(i, 0)->data().value<void*>();
...
private:
	...
	QString GetInterfaceVersionsString()
	{
        ...
        QString GetInterfaceVersionsString(Neurotec::Plugins::NPluginModule * module)
	{
		if(!module) return QString();
		int count;
                Neurotec::NVersionRange * versions = module->GetInterfaceVersions(&count);
		QString resul = InterfaceVersionsToString(versions, count);
		delete [] versions;
		return resul;
	}

        QString InterfaceVersionsToString(Neurotec::NVersionRange * pVersions, int count)
	{
		QString value;
        ...
        Neurotec::Plugins::NPlugin * SelectedPlugin()
	{
		...
		if(selected.count() != -1)
			return NULL;
                ...
                return (Neurotec::Plugins::NPlugin*)selected.at(0).data().value<void*>();
	}
	...
	{
		QStandardItemModel * model = (QStandardItemModel*)tvPlugins->model();
                Neurotec::Plugins::NPlugin * plugin = (Neurotec::Plugins::NPlugin*)model->item(row, 0)->data().value<void*>();
        ...
        void UpdatePlugin(Neurotec::Plugins::NPlugin * plugin)
	{
		...
		for(int i = 0; i < model->rowCount(); i++)
		{
                        Neurotec::Plugins::NPlugin * item = (Neurotec::Plugins::NPlugin*)model->item(i)->data().value<void*>();
	...
	static void PluginManager_PluginAdded(Neurotec::QNObjectBase * pObject, Neurotec::Collections::NCollectionChangedAction action,
                                                                                  int newIndex, Neurotec::Plugins::NPlugin * const * arNewItems, int newItemsCount,
                                                                                  int oldIndex, Neurotec::Plugins::NPlugin * const * arOldItems, int oldItemsCount, void * pParam)
	{
		QPluginManagerDialog * dialog = (QPluginManagerDialog*)pParam;
		if (action == Neurotec::Collections::nccaAdd)
		{
			...
			{
                                QMetaObject::invokeMethod(dialog, "OnPluginAdded", Qt::QueuedConnection, Q_ARG(Neurotec::Plugins::NPlugin*, arNewItems[i]));
			}
		}
		Q_UNUSED(pObject);
		Q_UNUSED(newIndex);
	...
	{
		QPluginManagerDialog * dialog = (QPluginManagerDialog*)pParam;
                QMetaObject::invokeMethod(dialog, "OnPluginChanged", Qt::QueuedConnection, Q_ARG(Neurotec::Plugins::NPlugin*, (Neurotec::Plugins::NPlugin*)pObject));
	...
	void OnSelectedPluginChanged()
	{
		...
		int sc = selected.count();
                Neurotec::Plugins::NPlugin * plugin = sc == 1 ? (Neurotec::Plugins::NPlugin*)selected.at(0).data().value<void*>() : NULL;

		QStandardItemModel * model = (QStandardItemModel*)tvActivated->model();
		model->removeRows(0, model->rowCount());
                Neurotec::Plugins::NPluginModule * module = plugin == NULL? NULL: plugin->GetModule();
		if (module)
		{
	...
	void OnPluginManagerChanged()
	{
		...
		{
			int count;
                        Neurotec::Plugins::NPlugin ** arPlugins = pluginManager->GetPlugins()->ToArray(&count);
                        auto_array<Neurotec::Plugins::NPlugin*> plugins(arPlugins, count);
			for(int i = 0; i < count; i++)
			{
				AddPlugin(plugins[i]);
			}
...
private slots:
	void UpdateTotalInfo()
	{
		if (pluginManager)
		{
			...
			for(int i = 0; i < count; i++)
			{
                                Neurotec::Plugins::NPlugin * plugin = (Neurotec::Plugins::NPlugin*)model->item(i)->data().value<void*>();
        ...
        void OnSetSelectedPlugin(Neurotec::Plugins::NPlugin* select)
	{
		...
		{
                        QMetaObject::invokeMethod(this, "OnSetSelectedPlugin", Qt::BlockingQueuedConnection, Q_ARG(Neurotec::Plugins::NPlugin*, select));
			return;
		}
		...
		if(select)
		{
			...
			for(int i = 0; i < model->rowCount(); i++)
			{
				QStandardItem * item = model->item(i);
                                Neurotec::Plugins::NPlugin * plugin = (Neurotec::Plugins::NPlugin*)item->data().value<void*>();
        ...
        void OnPluginAdded(Neurotec::Plugins::NPlugin * plugin)
	{
		if (plugin)
		{
			...
			{
				QStandardItem * item = model->item(i);
                                Neurotec::Plugins::NPlugin * p = (Neurotec::Plugins::NPlugin*)item->data().value<void*>();
