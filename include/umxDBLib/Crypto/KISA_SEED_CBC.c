...
// Set up input values for Key Schedule	
	A = ((unsigned int *)pbszUserKey)[0];
	B = ((unsigned int *)pbszUserKey)[1];
	C = ((unsigned int *)pbszUserKey)[2];
	D = ((unsigned int *)pbszUserKey)[3];
...
int SEED_CBC_Decrypt( IN unsigned char *pbszUserKey, IN unsigned char *pbszIV, IN unsigned char *pbszCipherText, IN int nCipherTextLen, OUT unsigned char *result )
{
	...
	newpbszCipherText = (unsigned char *)malloc(sizeof(unsigned char) * (nCipherTextLen));
	memcpy(newpbszCipherText, pbszCipherText, nCipherTextLen);

	pbszPlainText = (unsigned char *)malloc(sizeof(unsigned char) * (nCipherTextLen));
...
unsigned char pbszUserKey[16] = "19791008bkko@kr";
unsigned char pbszIV[16] = {0x026, 0x08d, 0x066, 0x0a7, 0x035, 0x0a8, 0x01a, 0x081, 0x06f, 0x0ba, 0x0d9, 0x0fa, 0x036, 0x016, 0x025, 0x001};

void SEED_SetKey(IN unsigned char* key)
{
	memcpy(pbszUserKey, key, 16);
}

int SEED_Encrypt4(IN unsigned char* plainText, int plainSize, OUT unsigned char* cipherText)
{
    unsigned char pbszCipherText[BUFSIZ] = {0x00};
    unsigned char pbszPlainText[BUFSIZ] = {0x00};
    KISA_SEED_INFO info;
    int i;
    int nPlainTextLen = 0;
    ...
    unsigned char *CipherBytes = (unsigned char *)(pbszCipherText);
    unsigned int *data;
    unsigned char *cdata;
    int nRetOutLeng = 0;
    int remainleng = 0;
    ...
    //printf ("\n\nLength of Plaintext : 10");
    //scanf("%d", &nPlainTextLen);
    //nPlainTextLen = (int)strlen((char*)plainText);
    nPlainTextLen = plainSize;

//    printf ("\nSEED_Encrypt-Plaintext(%d) : ", nPlainTextLen);
//    for (i=0;i<nPlainTextLen;i++)	{printf("%02X",plainText[i]);}
    ...
    //printf ("\n\nLength of Ciphertext : %d", nCipherTextLen);
    ...
    //printf ("\nCiphertext(%d) : ", nCipherTextLen);
    //for (i=0;i<nCipherTextLen;i++)	{printf("%d ",(int)CipherBytes[i]);}

    //cipherText = CipherBytes;
    memcpy(cipherText, CipherBytes, nCipherTextLen);

//    printf ("\nSEED_Encrypt-ciphertext(%d) : ", nCipherTextLen);
//    for (i=0;i<nCipherTextLen;i++)	{printf("%02X",CipherBytes[i]);}
...
int SEED_Decrypt(IN unsigned char* cipherText, OUT unsigned char* plainText, int cipherlength)
{
	...
	int EncryptedMessage_length = 0;
	unsigned int *outbuf;
	unsigned char *PlainBytes = plainText;
	unsigned char *CipherBytes = cipherText;
	unsigned int *data;
	unsigned char *cdata;
	int nRetOutLeng = 0;
	int remainleng = 0;
...
//    for (i=0;i<16;i++)	{std::cout << pbszUserKey << std::endl;}


//    //printf ("\nIV       : ");
//    //for (i=0;i<16;i++)	{printf("%02X ",pbszIV[i]);}
//    std::cout << "IV       : " << std::endl;
//    for (i=0;i<16;i++)	{std::cout << pbszIV << std::endl;}

	nCipherTextLen = cipherlength;

//    //printf ("\nCiphertext(%d) : ", nCipherTextLen);
//    //for (i=0;i<nCipherTextLen;i++)	{printf("%02X ",CipherBytes[i]);}
//    std::cout << "Ciphertext : " << nCipherTextLen << std::endl;
//    for (i=0;i<nCipherTextLen;i++)	{std::cout << CipherBytes << std::endl;}

//    //printf ("\n\nLength of Ciphertext : %d", nCipherTextLen);
//    //scanf("%d", &nCipherTextLen);

//    printf ("\nSEED_Decrypt-Ciphertext(%d) : ", nCipherTextLen);
//    for (i=0;i<nCipherTextLen;i++)	{printf("%02X",CipherBytes[i]);}


//    std::cout << "nCiphertext : " << nCipherTextLen << std::endl;
//    for (i=0;i<nCipherTextLen;i++)	{std::cout << CipherBytes << std::endl;}
...
//    printf ("\nSEED_Decrypt-Plaintext(%d) [%s] : ", nPlainTextLen, PlainBytes);
//    for (i=0;i<nPlainTextLen;i++)	{printf("%02X",PlainBytes[i]);}

    memcpy(plainText, PlainBytes, nPlainTextLen);
...
void main()
{
	...
	unsigned char pbszUserKey[16] = {0x088, 0x0e3, 0x04f, 0x08f, 0x008, 0x017, 0x079, 0x0f1, 0x0e9, 0x0f3, 0x094, 0x037, 0x00a, 0x0d4, 0x005, 0x089}; 
	unsigned char pbszIV[16] = {0x026, 0x08d, 0x066, 0x0a7, 0x035, 0x0a8, 0x01a, 0x081, 0x06f, 0x0ba, 0x0d9, 0x0fa, 0x036, 0x016, 0x025, 0x001};
